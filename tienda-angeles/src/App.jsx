...
function QRScanner({ onScan, onClose }) {
      ...
      try {
        ...
        if (videoRef.current) {
          ...
          if ("BarcodeDetector" in window) {
            ...
            setError("Tu navegador no soporta escaneo nativo. Ingresá el producto manualmente.");
          }
        }
      } catch {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.93)", zIndex: 1000, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: "#e8c07d", fontFamily: "sans-serif", marginBottom: 16, fontSize: 15, letterSpacing: 1, fontWeight: 600 }}>
        📷 Apuntá al código QR de la etiqueta
      </div>
      {error ? (
        <div style={{ color: "#FF8C00", background: "rgba(255,140,0,0.12)", padding: "16px 24px", borderRadius: 12, maxWidth: 300, textAlign: "center", fontSize: 14, border: "1px solid rgba(255,140,0,0.3)" }}>
          {error}
        </div>
      ) : (
        <div style={{ position: "relative", width: 280, height: 280, borderRadius: 16, overflow: "hidden" }}>
          <video ref={videoRef} style={{ width: "100%", height: "100%", objectFit: "cover" }} muted playsInline />
          <div style={{ position: "absolute", top: 10, left: 10, width: 28, height: 28, borderTop: "4px solid #e8c07d", borderLeft: "4px solid #e8c07d", borderRadius: "3px 0 0 0" }} />
          <div style={{ position: "absolute", top: 10, right: 10, width: 28, height: 28, borderTop: "4px solid #e8c07d", borderRight: "4px solid #e8c07d", borderRadius: "0 3px 0 0" }} />
          <div style={{ position: "absolute", bottom: 10, left: 10, width: 28, height: 28, borderBottom: "4px solid #e8c07d", borderLeft: "4px solid #e8c07d", borderRadius: "0 0 0 3px" }} />
          <div style={{ position: "absolute", bottom: 10, right: 10, width: 28, height: 28, borderBottom: "4px solid #e8c07d", borderRight: "4px solid #e8c07d", borderRadius: "0 0 3px 0" }} />
      ...
      <button onClick={onClose} style={{ marginTop: 28, background: "rgba(255,255,255,0.13)", color: "#fff", border: "none", padding: "12px 32px", borderRadius: 30, fontSize: 15, cursor: "pointer" }}>
        Cancelar
      </button>
...
export default function App() {
  ...
  const [form, setForm] = useState({
    ...
    urgencia: "media", vendedor: "",
    tieneSeña: false, comprobante: null, comprobanteNombre: "",
  });
  ...
  const handleQRScan = (raw) => {
    ...
    // Formato esperado del QR: "Nombre del producto|$1500" o solo "Nombre"
  ...
  const enviarFaltante = () => {
    if (!form.producto || !form.vendedor || !form.cantidad) {
      mostrarNotif("Completá producto, vendedor y cantidad", "err"); return;
    }
    if (form.tieneSeña && !form.comprobante) {
      mostrarNotif("Adjuntá la foto de la factura para registrar la seña", "err"); return;
    }
    const nuevo = {
      id: Date.now(), ...form,
      fecha_reporte: new Date().toISOString().slice(0, 10),
      estado: "pendiente", fecha_llegada: null, nota_compras: "",
    };
    setFaltantes(prev => [nuevo, ...prev]);
  ...
  const actualizarFaltante = (id) => {
    setFaltantes(prev => prev.map(f => f.id === id ? { ...f, ...formCompras } : f));
    setEditando(null);
  ...
  const inp = (extra = {}) => ({
    style: {
      ...
      border: "2px solid #E8E4DF", fontSize: 15, outline: "none", fontFamily: "inherit",
      ...extra,
    },
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        .bh{transition:all .22s;cursor:pointer;border:none}
        .bh:hover{transform:translateY(-3px);box-shadow:0 14px 40px rgba(0,0,0,.45)}
      `}</style>
        ...
        {!pedirPassword ? (
          ...
          <div style={{ background: "rgba(255,255,255,0.07)", borderRadius: 20, padding: 28, maxWidth: 340, margin: "0 auto" }}>
            ...
            <input
              ...
              onKeyDown={e => {
                if (e.key === "Enter") {
                  if (passwordInput === "comprastienda") { setPedirPassword(false); setPasswordInput(""); setRol(ROLES.COMPRAS); }
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
              <button className="bh" onClick={() => { setPedirPassword(false); setPasswordInput(""); setErrorPassword(false); }}
                style={{ flex: 1, padding: "11px", borderRadius: 10, background: "rgba(255,255,255,0.1)", color: "#ccc", fontSize: 14, fontWeight: 600 }}>
                Volver
              </button>
              <button className="bh" onClick={() => {
                if (passwordInput === "comprastienda") { setPedirPassword(false); setPasswordInput(""); setRol(ROLES.COMPRAS); }
                else setErrorPassword(true);
              }} style={{ flex: 1, padding: "11px", borderRadius: 10, background: "linear-gradient(135deg,#4a9eff,#2176d4)", color: "#fff", fontSize: 14, fontWeight: 700 }}>
                Ingresar
              </button>
            </div>
          </div>
    ...
    <div style={{ minHeight: "100vh", background: "#F7F5F2", fontFamily: "'Source Sans 3',Georgia,sans-serif" }}>
      <style>{`
        ...
        *{box-sizing:border-box}
        .ba{transition:all .15s;cursor:pointer;border:none}
        .ba:hover{opacity:.85;transform:translateY(-1px)}
        ...
        {vista === "nuevo" && !esCompras && (
          <div style={{ background: "#fff", borderRadius: 18, padding: 22, boxShadow: "0 2px 20px rgba(0,0,0,.08)", marginBottom: 24 }}>
            ...
            <div style={{ display: "grid", gap: 14 }}>
              ...
              {form.precio && (
                <div style={{ background: "#F0FFF6", border: "1.5px solid #22AA66", borderRadius: 8, padding: "8px 14px", fontSize: 14, color: "#22AA66", fontWeight: 600 }}>
                  💰 Precio del QR: {form.precio}
                </div>
              )}
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {Object.entries(URGENCIA_CONFIG).map(([key, cfg]) => (
                    <button key={key} className="ba" onClick={() => setForm({ ...form, urgencia: key })} style={{
                      flex: 1, padding: "10px 0", borderRadius: 10, fontWeight: 600, fontSize: 13,
                      background: form.urgencia === key ? cfg.bg : "#F7F5F2",
                      color: form.urgencia === key ? cfg.color : "#888",
                      border: `2px solid ${form.urgencia === key ? cfg.color : "transparent"}`,
                    }}>{cfg.label}</button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ borderTop: "2px dashed #E8E4DF", paddingTop: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }}
                  onClick={() => setForm(f => ({ ...f, tieneSeña: !f.tieneSeña, comprobante: null, comprobanteNombre: "" }))}>
                ...
                {form.tieneSeña && (
                  <div style={{ marginTop: 14, display: "grid", gap: 12 }}>
                    <div style={{ background: "#FFF8F0", border: "1.5px solid #FF8C00", borderRadius: 10, padding: "10px 14px", fontSize: 13, color: "#AA5500" }}>
                      ⚠️ Adjuntá la foto de la <b>factura del ERP</b> como comprobante.
                    </div>
                    ...
                    {!form.comprobante ? (
                      <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, padding: "20px 14px", borderRadius: 12, border: "2px dashed #c9943c", cursor: "pointer", background: "#FFFBF5", color: "#c9943c", fontWeight: 600, fontSize: 14, textAlign: "center" }}>
                        ...
                        Tocá para sacar foto o elegir de galería
                      ...
                      <div style={{ position: "relative" }}>
                        <img src={form.comprobante} alt="Factura" onClick={() => setImagenVista(form.comprobante)}
                          style={{ width: "100%", maxHeight: 200, objectFit: "cover", borderRadius: 10, cursor: "zoom-in", border: "2px solid #22AA66", display: "block" }} />
                        <div style={{ position: "absolute", top: 8, right: 8, display: "flex", gap: 6 }}>
                          ...
                          <button className="ba" onClick={e => { e.stopPropagation(); setForm(f => ({ ...f, comprobante: null, comprobanteNombre: "" })); }}
                            style={{ background: "#FF4444", color: "#fff", borderRadius: 20, padding: "4px 10px", fontSize: 12, fontWeight: 700 }}>✕</button>
                        </div>
                        <div style={{ textAlign: "center", fontSize: 11, color: "#aaa", marginTop: 4 }}>👆 Tocá para ver en grande</div>
                      </div>
                    )}
                  </div>
                )}
        ...
        {vista === "lista" && (
          <>
            ...
            {filtrados.map(f => {
              ...
              return (
                <div key={f.id} className="card" style={{ background: "#fff", borderRadius: 16, padding: 18, marginBottom: 14, boxShadow: "0 2px 12px rgba(0,0,0,.06)", borderLeft: `4px solid ${urg.color}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 6, flexWrap: "wrap" }}>
                        ...
                        <span style={{ background: urg.bg, color: urg.color, padding: "2px 9px", borderRadius: 20, fontSize: 11, fontWeight: 700 }}>{urg.label}</span>
                        <span style={{ background: est.bg, color: est.color, padding: "2px 9px", borderRadius: 20, fontSize: 11, fontWeight: 700 }}>{est.label}</span>
                      ...
                      {f.tieneSeña && f.comprobante && (
                        <div style={{ marginTop: 12 }}>
                          ...
                          <img src={f.comprobante} alt="Comprobante" onClick={() => setImagenVista(f.comprobante)}
                            style={{ width: "100%", maxHeight: 160, objectFit: "cover", borderRadius: 8, cursor: "zoom-in", border: "2px solid #e8c07d", display: "block" }} />
                          <div style={{ fontSize: 11, color: "#aaa", marginTop: 3 }}>👆 Tocá para ver en grande</div>
                        </div>
                      )}
                    </div>
                  ...
                  {esCompras && editando === f.id && (
                    <div style={{ marginTop: 16, paddingTop: 16, borderTop: "2px dashed #E8E4DF", display: "grid", gap: 12 }}>
                      <div>
                        ...
                        <select value={formCompras.estado} onChange={e => setFormCompras({ ...formCompras, estado: e.target.value })}
                          style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "2px solid #E8E4DF", fontSize: 15, background: "#fff" }}>
                      ...
                      <div>
                        ...
                        <input type="date" value={formCompras.fecha_llegada} onChange={e => setFormCompras({ ...formCompras, fecha_llegada: e.target.value })}
                          style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "2px solid #E8E4DF", fontSize: 15 }} />
                      </div>
                      <div>
                        <label style={{ fontSize: 13, fontWeight: 600, color: "#555", display: "block", marginBottom: 6 }}>Nota para el equipo</label>
                        <textarea value={formCompras.nota_compras} onChange={e => setFormCompras({ ...formCompras, nota_compras: e.target.value })}
                          placeholder="Ej: Pedido a Textil Norte, llega el jueves..." rows={2}
                          style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "2px solid #E8E4DF", fontSize: 14, resize: "none" }} />
                      </div>
