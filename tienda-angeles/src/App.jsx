      ...
      <div style={{ textAlign: "center", maxWidth: 440, width: "100%" }}>
        ...
        <p style={{ color: "#a0b4cc", fontFamily: "'Source Sans 3',sans-serif", fontSize: 15, marginBottom: 48 }}>Sistema de gestión de faltantes</p>
            ...
            <div style={{ display: "flex", gap: 16, justifyContent: "center" }}>
              ...
              <button className="bh" onClick={() => { localStorage.setItem('tienda_rol', ROLES.COMPRAS); setRol(ROLES.COMPRAS); }} style={{ padding: "20px 30px", background: "linear-gradient(135deg,#7ec8e3,#3a8fb5)", borderRadius: 16, color: "#1a1a2e", fontFamily: "'Playfair Display',serif", fontSize: 18, fontWeight: 700, minWidth: 150 }}>
          ...
          <>
            <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
              {[["todos","Todos"],["pendiente","Pendientes"],["en_proceso","En proceso"],["comprado","Comprado"],["recibido","Recibidos"],["sin_stock","No hay en stock"]].map(([val, label]) => (
                <button key={val} className="ba" onClick={() => irAFiltro(val)} style={{ padding: "7px 14px", borderRadius: 20, fontSize: 13, fontWeight: 600, background: filtroEstado === val ? "#1a1a2e" : "#fff", color: filtroEstado === val ? "#fff" : "#666", boxShadow: "0 1px 6px rgba(0,0,0,.07)" }}>
                  {label}
                </button>
              ))}
            </div>
